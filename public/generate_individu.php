...
if (isset($_GET['lokasi']) && isset($_GET['where'])) {
    ...
    // mysqli_query($koneksi, "CREATE TABLE IF NOT EXISTS rencana_angsuran_i_$lokasi LIKE rencana_angsuran_i_1");
    // mysqli_query($koneksi, "CREATE TABLE IF NOT EXISTS real_angsuran_i_$lokasi LIKE real_angsuran_i_1");

    $total = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM pinjaman_anggota_$lokasi WHERE (status='A' OR status='W' OR status='L' OR status='H' OR status='R') 
                        AND ($limit) AND (jenis_pinjaman = 'I') ORDER BY id ASC"));
    echo "Total Pinjaman anggota Lokasi $lokasi = " . $total . "<br><br>";
...
?>
    ...
    $pinjaman_kelompok = mysqli_query($koneksi, "SELECT * FROM pinjaman_anggota_$lokasi WHERE (status='A' OR status='W' OR status='L' OR status='H' OR status='R') 
                        AND ($limit) AND (jenis_pinjaman = 'I') ORDER BY id ASC LIMIT $start, $per_page");
    ...
    while ($pk = mysqli_fetch_array($pinjaman_kelompok)) {
        $ang = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM anggota_$lokasi WHERE id='$pk[nia]'"));
        $desa = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM desa WHERE kd_desa='$ang[desa]'"));
        $del_re = mysqli_query($koneksi, "DELETE FROM real_angsuran_i_$lokasi WHERE loan_id=$pk[id]");
        $del_ra = mysqli_query($koneksi, "DELETE FROM rencana_angsuran_i_$lokasi WHERE loan_id=$pk[id]");
        ...
        if (mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM rencana_angsuran_i_$lokasi WHERE loan_id='$pk[id]' AND angsuran_ke='0'")) <= 0) {
            // Insert Rencana Angsuran Ke-0
            $input_ra = mysqli_query($koneksi, "INSERT INTO `rencana_angsuran_i_$lokasi`
        ...
        for ($ke = 1; $ke <= $pk['jangka']; $ke++) {
            ...
            $tarj = $tarj + $wajas;
            $ra = mysqli_query($koneksi, "SELECT * FROM rencana_angsuran_i_$lokasi WHERE loan_id='$pk[id]' AND jatuh_tempo='$tglang'");

            // Cek apakah data yang akan di input sudah ada didalam tabel rencana_angsuran_i_$lokasi
            if (mysqli_num_rows($ra) <= 0) {

                // Insert data kedalam tebel rencana_angsuran dengan angsuran_ke>0
                $sqlinput = mysqli_query($koneksi, "INSERT INTO `rencana_angsuran_i_$lokasi`(`id`, `loan_id`, `angsuran_ke`,
        ...
        $tansaksi_loan = mysqli_query($koneksi, "SELECT * FROM transaksi_$lokasi WHERE id_pinj_i='$pk[id]' AND idtp!='0' ORDER BY tgl_transaksi ASC, idtp ASC ");
        $sum_pokok = 0;
        $sum_jasa = 0;
        echo $pk['id'];
        echo " = ";
        ...
        while ($tr = mysqli_fetch_array($tansaksi_loan)) {
            ...
            $real = mysqli_query($koneksi, "SELECT * FROM real_angsuran_i_$lokasi WHERE loan_id='$id_pinj' ORDER BY tgl_transaksi DESC, id DESC");
            ...
            $ra = mysqli_fetch_array(mysqli_query($koneksi, "select * from rencana_angsuran_i_$lokasi WHERE loan_id='$id_pinj' AND jatuh_tempo<='$tgl_transaksi' 
            ...
            $realisasi = mysqli_query($koneksi, "SELECT * FROM real_angsuran_i_$lokasi WHERE loan_id='$pk[id]' AND id='$tr[idtp]'");
            ...
            if (mysqli_num_rows($realisasi) <= 0) {
                ...
                if (!$sqlinput2) {
                    ...
                    echo "<br>";
                    var_dump("INSERT INTO `real_angsuran_i_$lokasi` (`id`, `loan_id`, `tgl_transaksi`, `realisasi_pokok`, `realisasi_jasa`, 
                    `sum_pokok`, `sum_jasa`, `saldo_pokok`, `saldo_jasa`, `tunggakan_pokok`, `tunggakan_jasa`, `lu`, `id_user`) VALUES ('$tr[idtp]','$pk[id]',
                    '$tgl_transaksi','$realisasi_pokok','$realisasi_jasa','$sum_pokok','$sum_jasa','$saldo_pokok','$saldo_jasa','$tunggakan_pokok','$tunggakan_jasa',
                    '$datetime','1')");
                    die;
